...
export const userProfileData = async (req, res) => {
  try {
    ...
    return res.json({ message: "user profile data updated" });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
...
export const sendConnectionRequest = async (req, res) => {
  ...
  try {
    ...
    if (!connectionUser)
      return res
        .status(404)
        .json({ message: "connection user does not exist" });
    ...
    await request.save();
    return res.json({ message: "Request sucessfully sent" });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
...
export const commentPost = async (req, res) => {
  const { token, post_id, comment: commentText } = req.body;
  try {
